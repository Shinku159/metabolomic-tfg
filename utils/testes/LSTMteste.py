...
dataset = pd.read_csv("https://www.apmonitor.com/do/uploads/Main/tclab_dyn_data3.txt")

s1 = MinMaxScaler(feature_range=(-1, 1))
xNorm = s1.fit_transform(dataset[["T1", "Q1"]])
s2 = MinMaxScaler(feature_range=(-1, 1))
yNorm = s2.fit_transform(dataset[["Q1"]])
...
modelLSTM.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3, decay=1e-5),
              ...
              metrics=['accuracy'])
history = modelLSTM.fit(X, Y, epochs=10, batch_size=250, verbose=1)
