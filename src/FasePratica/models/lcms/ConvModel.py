...
class ConvModel(tf.keras.Model):
  ...
  **kwargs: Json object,
  ...
  def __init__(self, inputShape, outputShape, **kwargs):
    ...
    self.conv1 = tf.keras.layers.Conv1D(filters=128, kernel_size=(3), activation=tf.nn.relu, padding='same')
    self.drop1 = tf.keras.layers.Dropout(0.2)
    self.conv2 = tf.keras.layers.Conv1D(filters=128, kernel_size=(3), activation=tf.nn.relu, padding='same')
    self.drop2 = tf.keras.layers.Dropout(0.2)
    self.conv3 = tf.keras.layers.Conv1D(filters=128, kernel_size=(3), activation=tf.nn.relu, padding='same')
    self.drop3 = tf.keras.layers.Dropout(0.2)
    self.conv4 = tf.keras.layers.Conv1D(filters=128, kernel_size=(3), activation=tf.nn.relu, padding='same')
    self.drop4 = tf.keras.layers.Dropout(0.2)
    self.conv5 = tf.keras.layers.Conv1D(filters=128, kernel_size=(3), activation=tf.nn.relu, padding='same')
