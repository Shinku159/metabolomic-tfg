...
class LstmModel(tf.keras.Model):
  ...
  **kwargs: Json object,
  ...
  def __init__(self, inputShape, outputShape, **kwargs):
    ...
    self.lstm1 =  tf.keras.layers.LSTM(units=1024, activation=tf.nn.relu, return_sequences=True)
    self.drop1 = tf.keras.layers.Dropout(0.2) 
    self.lstm2 =  tf.keras.layers.LSTM(units=512, activation=tf.nn.relu, return_sequences=True)
    self.drop2 = tf.keras.layers.Dropout(0.2) 
    self.lstm3 =  tf.keras.layers.LSTM(units=256, activation=tf.nn.relu, return_sequences=True)
    self.drop3 = tf.keras.layers.Dropout(0.2) 
    self.lstm4 =  tf.keras.layers.LSTM(units=128, activation=tf.nn.relu, return_sequences=True)
    self.drop4 = tf.keras.layers.Dropout(0.2) 
    self.lstm5 =  tf.keras.layers.LSTM(units=64, activation=tf.nn.relu, return_sequences=True)
    self.drop5 = tf.keras.layers.Dropout(0.2) 
    self.lstm6 =  tf.keras.layers.LSTM(units=32, activation=tf.nn.relu, return_sequences=False)
