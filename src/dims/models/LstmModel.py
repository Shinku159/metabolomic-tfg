...
class LstmModel(tf.keras.Model):
  ...
  **kwargs: Json object,
  ...
  def __init__(self, inputShape, outputShape, **kwargs):
    ...
    self.inputs =  tf.keras.layers.LSTM(units=128, input_shape=inputShape, activation=tf.nn.relu, return_sequences=True)
    self.drop = tf.keras.layers.Dropout(0.2) 
    self.lstm1 =  tf.keras.layers.LSTM(units=256, activation=tf.nn.relu, return_sequences=True)
    self.drop1 = tf.keras.layers.Dropout(0.2) 
    self.lstm2 =  tf.keras.layers.LSTM(units=256, activation=tf.nn.relu, return_sequences=True)
    self.drop2 = tf.keras.layers.Dropout(0.2)
    self.lstm3 = tf.keras.layers.LSTM(units=128, activation=tf.nn.relu, return_sequences=False)
